...
insert into `MOVIESTAR` values("Zahari Baharov", "U fakultetu", "M","1977-08-08"); 

delete from `STUDIO` where `ADDRESS` like "%5%" 

update `MOVIE` set `STUDIONAME` = "Fox" where `TITLE` like "%star%" 

select starname from `STARSIN` where `MOVIETITLE` = "Terms of Endearment"; 

select starname from `STARSIN`, 'STUDIO' where `MOVIEYEAR` = "1995" and Studio.name = "MGM"  

alter table `STUDIO` add column PresidentName varchar(100) 

update `STUDIO` set `PresidentName` = "Ivan Stoqnov" where `NAME` = "MGM" 

SELECT `TITLE` FROM `MOVIE` WHERE `LENGTH` >  (SELECT `LENGTH` FROM `MOVIE` where `TITLE` = "Gone With the Wind");

select `TITLE` from `MOVIE`,`MOVIEEXEC` where `MOVIE`.`PRODUCER`> (Select `PRODUCER` from `MOVIE` where `NAME` = "Merv Griffin");  

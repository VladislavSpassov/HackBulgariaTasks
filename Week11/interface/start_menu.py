...
3 - exit
        ...
        if start_option == '1':
            ...
            is_doctor = input("Are you doctor?: ")
            status = "Patient"
            if(is_doctor == "yes"):
                status = "Doctor"
                title = input("Provide your title")

                current_user = MainController.sign_up(
                    username, password, second_password,status,title = title)
            elif(is_doctor == "no"):
                age = input("Provide your age:")
                full_name = input("Provide your full name:")
                address = input("Provide your addrees:")
                health_status = input("Provide your health status:")
                current_user = MainController.sign_up(username,password,second_password,status, age = age,full_name = full_name,address = address, health_status = health_status)
            else:
                print("Invalid input")
                sys.exit(1)
